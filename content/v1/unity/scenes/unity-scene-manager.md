...
Scene Load Timeout controls how long the authority waits for a client to answer a scene load request before treating the load as failed. Zero or less waits indefinitely.
...
`TryGetScene` resolves the live Unity scene that an authority-opened instance became on this peer. It returns false while the scene is still loading, and false on a peer the instance was never opened for. A handle can be known - the authority already told this peer about the instance - before its `Scene` exists locally, because the load is still in progress; that is the gap this method reports.
