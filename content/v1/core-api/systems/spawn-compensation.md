...
- `Authority` — the authority catches the object up through the latency a client's spawn *request* spent reaching it. Only a client-predicted spawn has lost any time getting to the authority; the authority's own spawns are on time by definition and this scope compensates nothing for them.
...
`NetworkSystem.MaximumTransmittedSpawnCompensationTicks` is 63 — the most ticks of already-spent authority-side compensation the wire can carry when it tells a remote client how much of the object's age the authority already accounted for. A budget larger than that is clamped to 63 ticks for transmission, independent of the millisecond cap.
