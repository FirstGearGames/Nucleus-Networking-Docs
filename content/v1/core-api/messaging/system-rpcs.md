...
- `RpcTarget` is who the call is for: `RpcTarget.Server` (client to authority only), `RpcTarget.Observers` (fan-out to every observer), `RpcTarget.ObserversExcept(connection, isControllerExcluded)` (authority only), or `RpcTarget.To(connection)` (one named connection).
...
The return value reports only what this peer can see: a client cannot know the authority's observer set, so a call it routed returning `true` means it was queued, not that it was delivered.
...
| `IsSenderServer` | Whether the authority is what sent this call. |
