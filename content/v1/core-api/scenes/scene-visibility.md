...
`SpawnGate` only carries `InterestEffect.Spawn`, but it is not just a spawn-time check. `Connection.IsSceneLoaded` decides whether the client currently holds the scene, and a client stops holding it the moment `SceneManager.RequestSceneUnload` marks it as leaving — not when the unload finishes. `RequestSceneUnload` calls `SystemManager.RemoveSceneObservers` immediately, dropping the connection from that scene's systems before any unload confirmation comes back. `SceneInterestCondition.Evaluate` mirrors this: once `Connection.IsSceneUnloadRequested` is true for that scene, the condition returns `InterestResult.Spawn(isMet: false)` instead of abstaining, withholding the object rather than re-serving it into a scene the client is on its way out of.
...
- If the object is the client's own controlled object arriving right as it enters the scene, confirm `SceneManager.IsSystemCarriedFor` conditions hold: the system must be carry-eligible, the scene load must be outstanding for that connection, and the connection must control the system's group.
- On a genuinely local setup — an emulated connection or a peer sharing an in-memory Transport — `SpawnGate` abstains (`connection.IsLocalPeer`); this does not apply to a real host's own client, which is gated normally and reported through `NetworkSystem.HostInterestMembership` instead.
