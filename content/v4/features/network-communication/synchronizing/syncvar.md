...
SyncTypes can also be customized with additional options by using the `UpdateSettings` method within your declared SyncTypes, or using the initializer of your SyncType. These options include being notified when the value changes, changing how often the SyncType will synchronize, and more.
...
> **Important:** When modifying SyncVars using client-side, a RPC is sent with every set. If your SyncVar value will change frequently consider limiting how often you set the value client-side to reduce bandwidth usage.
...
[ServerRpc]
private void SetName(string value) => Name.Value = value;
```
...
// here for simplicity.
private readonly SyncVar<string> Name = new SyncVar<string>(new SyncTypeSettings(
    WritePermission.ClientUnsynchronized, ReadPermission.ExcludeOwner));

// Create a ServerRpc to allow owner to update the value on the server.
// RunLocally = true so the calling client also sets the value locally.
